...
<div class=\"EES_CSSico\" style=\"background-position:-520px\" title=\"本地化所有图片\" onClick=\"EES_Piclocal()\"></div>";
//--------精简版可删■■■↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
...
//--------精简版可删■■■↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
//批量上传编辑器中的图片
function EES_Piclocal(){
var eesfnm=document.getElementById(EES_ESan);
var eesfstr=eesfnm.value;
var eesimg='';
var eesimgj=0;
var eesimgreg = /<img.*?(?:>|\/>)/gi;
var eesscrreg = /src=[\'\"]?([^\'\"]*)[\'\"]?/i;
var eesimgarr = eesfstr.match(eesimgreg);
//alert('所有已成功匹配图片的数组：'+eesimgarr);
for (var eesimi = 0; eesimi < eesimgarr.length; eesimi++) {
var eesscrarr = eesimgarr[eesimi].match(eesscrreg);
var eesscr=eesscrarr[1];
if(eesscr.indexOf("://")>1){
var eeshrr=eesscr.split("://");
if(eesimg.indexOf(eesscr)<0){
//ajax上传图片
if(window.ActiveXObject){asaiXML=new ActiveXObject("Microsoft.XMLHTTP");}
else if(window.XMLHttpRequest){asaiXML=new window.XMLHttpRequest();}
asaiXML.open("POST",EES_ESsm+EES_ESse.replace("Edit/","UpLoad/")+'?99|'+eesscr,false);
asaiXML.send(null);
var asaixstrq=unescape(asaiXML.responseText);
if(asaixstrq.indexOf("##")=='0'){
eesimgj=eesimgj+1;
eesimg+='\n'+eesscr;
eesfstr=eesfstr.replace(new RegExp(eesscr,'g'),asaixstrq.replace('##',''));
}else if(asaixstrq.indexOf("注册")>0){eesimgj=-1}
if(eesimi<1){document.getElementById("up").value=asaixstrq.replace('##','');document.getElementById("upimg").src=asaixstrq.replace('##','')}
//alert('已匹配的图片地址'+(eesimi+1)+'：'+eesscr+'储存到'+asaixstrq);
}
}
}
if(eesimgj<0){alert('提示:无权执行这个操作！');}
else if(eesimgj>0){
alert('成功上传'+eesimgj+'张图片：'+eesimg);
EES_EDWindow.document.body.innerHTML=eesfstr;
eesfnm.value=eesfstr;
}
}
//插入链接
...
EES_Infileva=EES_Infileva+'媒体地址：<br><input id="'+EES_ESid+'EES_Infilefile" style="width:180px;" value="http://"><br><iframe style="background-color:'+EES_ESqy+';height:22px;width:258px;" src="'+EES_ESsm+EES_ESse.replace("Edit/","UpLoad/")+'?0|4|0|1|3|0|'+EES_ESid+'EES_Infilefile|0|0|0|0.ees" frameborder="0" scrolling="no"></iframe>\
